...
use super::{MAX_REQUEST_AGE_MS, MAX_REQUEST_ENTRIES, model::RequestEntry, model::RequestStore};

static STORE: OnceLock<Mutex<RequestStore>> = OnceLock::new();

pub(super) fn request_store() -> &'static Mutex<RequestStore> {
    STORE.get_or_init(|| Mutex::new(RequestStore::default()))
}

pub(super) fn is_expired(entry: &RequestEntry, now_ms: u128) -> bool {
    now_ms.saturating_sub(entry.timestamp_ms) > MAX_REQUEST_AGE_MS
}

pub(super) fn prune_expired(store: &mut RequestStore, now_ms: u128) {
...
pub(super) fn show(request_id: &str) -> Result<Value, AppError> {
...
pub(super) fn list(limit: Option<u64>) -> Result<Value, AppError> {
    let mut store = request_store()
        .lock()
        .map_err(|_| AppError::internal("request store lock poisoned"))?;
    let now_ms = now_millis();
...
pub(super) fn response(request_id: &str) -> Result<Value, AppError> {
    let store = request_store()
        .lock()
        .map_err(|_| AppError::internal("request store lock poisoned"))?;
    let now_ms = now_millis();
...
pub(super) fn screenshot(request_id: &str, out_path: Option<String>) -> Result<Value, AppError> {
    let store = request_store()
        .lock()
        .map_err(|_| AppError::internal("request store lock poisoned"))?;
    let now_ms = now_millis();
