...
fn parses_screen_tokenize_with_active_window_id() {
    ...
    let command = parse_command(&args).expect("screen tokenize --active-window <id> should parse");
    match command {
        Command::ScreenTokenize {
            active_window,
            active_window_id,
...
#[test]
fn parses_screen_tokenize_with_journal() {
    let args = vec![
        "screen".to_string(),
        "tokenize".to_string(),
        "--journal".to_string(),
    ];
    let command = parse_command(&args).expect("screen tokenize --journal should parse");
    match command {
...
fn rejects_pointer_click_id_without_active_window() {
    ...
    let err = parse_command(&args).expect_err("pointer click --id must require --active-window");
...
fn parses_window_list() {
    let command =
        parse_command(&["window", "list"].map(str::to_string)).expect("window list should parse");
    assert!(matches!(command, Command::WindowList));
}

#[test]
fn parses_window_bounds_with_title() {
    let command = parse_command(&["window", "bounds", "--title", "Calculator"].map(str::to_string))
...
fn parses_window_focus_with_title() {
    let command = parse_command(&["window", "focus", "--title", "Reminders"].map(str::to_string))
