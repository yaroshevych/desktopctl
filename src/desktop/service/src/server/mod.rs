...
fn active_window_description() -> Result<Value, AppError> {
    ...
    let target = frontmost_pid
        ...
        .map(|window| WindowSummary {
            ...
            app: window.app,
            title: window.title,
            document_url: window.document_url,
...
#[cfg(target_os = "macos")]
pub(crate) fn capture_active_window_for_agent_launcher() -> Option<i64> {
    platform::ax::frontmost_app_pid()
}

#[cfg(target_os = "macos")]
pub(crate) fn resolve_agent_launcher_target(
    pid: i64,
) -> Result<platform::windowing::WindowInfo, AppError> {
    let bounds = platform::ax::focused_window_bounds_for_pid(pid);
